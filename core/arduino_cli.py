...
class ArduinoCLI:
    ...
    def upload(self, code: str, fqbn: str, port: str, programmer: str = "") -> Tuple[bool, str]:
        """Upload Arduino sketch with automatic fallback."""
        ...
        # Lista de estratégias de upload (tenta uma por uma)
        strategies = []
        
        # Estratégia 1: Upload padrão (sem programador) - SEMPRE tenta primeiro
        strategies.append({
            "name": "Default serial upload",
            "cmd": ["upload", "--fqbn", fqbn, "-p", port, "--input-dir", str(build_path)],
            "timeout": 120
        })
        
        # Estratégia 2: Upload com hex file específico
        hex_files = list(build_path.glob("*.hex")) + list(build_path.glob("*.bin"))
        if hex_files:
            strategies.append({
                "name": f"Upload via hex file ({hex_files[0].name})",
                "cmd": ["upload", "--fqbn", fqbn, "-p", port, "--input-file", str(hex_files[0])],
                "timeout": 120
            })
        
        # Estratégia 3: Upload com verbose para debug
        strategies.append({
            "name": "Upload with verbose output",
            "cmd": ["upload", "--fqbn", fqbn, "-p", port, "--input-dir", str(build_path), "--verbose"],
            "timeout": 120
        })
        
        # Estratégia 4: Se programador especificado, tenta por último
        if programmer:
            strategies.append({
                "name": f"Upload with programmer: {programmer}",
                "cmd": ["upload", "--fqbn", fqbn, "-p", port, "--programmer", programmer, "--input-dir", str(build_path)],
                "timeout": 120
            })
        ...
        for strategy in strategies:
            print(f"[UPLOAD] Trying: {strategy['name']}...")
            ok, msg = self.run_command(strategy["cmd"], timeout=strategy["timeout"])
            
            if ok:
                print(f"[UPLOAD] ✅ SUCCESS: {strategy['name']}")
                return True, f"Upload successful: {strategy['name']}"
            
            # Coletar erro para diagnóstico
            errors.append(f"{strategy['name']}: {msg[:200]}")
            print(f"[UPLOAD] ❌ Failed: {strategy['name']}")
            
            # Pequena pausa entre tentativas (libera a porta serial)
            import time
            time.sleep(1)
        
        # Todas as estratégias falharam - mensagem de erro amigável
        error_detail = "\n".join(errors[:3])
        
        error_message = f"""Upload failed after {len(strategies)} attempts!

    📋 Error details:
    {error_detail}

    💡 Troubleshooting:
    1. Check that the correct port is selected
    2. Close other programs using the port (Arduino IDE, Serial Monitor)
    3. Press RESET button on Arduino just before clicking Upload
    4. Check that the correct board type is selected
    5. Try a different USB cable (must be DATA cable)
    6. On Linux: sudo usermod -aG dialout $USER (then logout/login)
    7. On Windows: Check Device Manager for COM port conflicts

    🔧 Manual test:
    {self.cli_path} upload --fqbn {fqbn} -p {port} --input-dir {build_path}
    """
        
        return False, error_message


    def _reset_serial_port(self, port: str):
        """Reset serial port via DTR/RTS to trigger Arduino bootloader."""
        try:
            import serial
            ser = serial.Serial(port, 1200)  # 1200 baud ativa bootloader
            ser.dtr = False
            ser.rts = False
            time.sleep(0.1)
            ser.dtr = True
            ser.rts = True
            ser.close()
            time.sleep(1)  # Aguarda o bootloader iniciar
            print(f"[UPLOAD] DTR reset sent to {port} (bootloader should be active)")
            return True
        except Exception as e:
            print(f"[UPLOAD] Could not reset DTR on {port}: {e}")
            return False
